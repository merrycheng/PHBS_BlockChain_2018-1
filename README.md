...
By building a Proof-of-Ownership platform relying on a cryptographic file-encryption protocol, the integrity and copyrights of the files are preserved among all participants and at any time.


# 1. Introduction:

Digital files’ distribution on the internet is facing multiple challenges since external actors have found ways to copy, reproduce and illegally redistribute digital contents. Annual losses in sales generated by piracy in the United States only is estimated to be over $25 billion. Besides media industries and institutions, individuals are also impacted when sharing content online. Indeed, it happens that they lose control on private files which end up be abusively used by third parties or companies. Keeping track and ownership over  content shared online is consequently an important need. 
	
What is really needed is a safe operating environment into which files’ reproduction and distribution is controlled and restricted by their owner. Thus, right holders would be able to safely duplicate, distribute, and monetize their files on the web. By encrypting each file with a unique and brute-force resistant password safely kept into a decentralized public ledger prior any p2p transfer to a file’s final recipient, 2PS propose a solution to address this problem. 
...
## 2.1 Regular web-based platforms;

Today, popular file sharing platforms and related services are diverse and often specific to one particular sector (videos, music, documents or games). For almost all video streaming platforms (youtube, youku, dailymotion, Netflix…), third parties (softwares) exist and allow to quicly and easily download and duplicate contents. Itunes or QQmusic are, by themselves, allowing users to download music files that they can store and eventually duplicate and share with external and unsolicited parties (Family, friends, websites, p2p...). Besides, unauthorized streaming platforms fed by hackers flourish on internet, stolen pdf-newspapers are available sometimes even before the paper version is released, and, similarly, new video games are quickly “cracked” and shared over the internet. 

This illegal sharing activity is amplified by unregulated p2p protocols such as BitTorrent. Those protocols often deny responsibilities, arguing that any fraudulent use made of their platform (even if representing the majority of the files exchanged) is none of their concerns, as only being service providers. The lack of possibilities for regulations and the difficulty for user-tracking in such platform is made challenging. In an international context, it is particularly difficult for authorities to identify and convict involved individuals. First, because they are often physically or virtually (VPNs) out of their jurisdiction and second, because they are quickly replaced.

## 2.2 Existing Blockchain platforms:

A bunch of blockchain-based companies have been working on fighting piracy. [Steem](https://steem.com/) for instance, is accounting for the fact that right holders and content providers are not always compensated rightefully by streaming platforms and third parties. Indeed, such plaforms pre-negociate contracts with their owners which often does not represent how much the file will be viewed, used and downloaded afterwards. It led some artists to deny the services of platforms like spotify because of the gap between payment and profit retained. Steem aims to create a direct link between content providers and customers in order to protect the interests of both. Steem's blockchain track how much files are played and pay artists with its cryptocurrency accordingly. 
...
•	**Zero-fee contracts**: EOS architecture oblige users to "pay" a couple EOS for creating a new wallet. In reality, those EOS are allocated to the network for staking. Consequently, each user is a participant of the consensus, which eliminate the inbalance between stakers and non-stakers. This governance process allows EOS to provide smart contracts with zero fees.
...
2PS total supply has been fixed to 10,000,000,000 2PS with a sub unit of 10-9. Such a large supply aims to favor a wider distribution between users than it could be with lower amounts, helping to fight speculation and monopolies. The token is 90%* pre-minted (see 8.1) with an initial global distribution as follow:
...
The 2PS platform is built with AI and machine learning algorithms, here to determine if an uploaded content is unique before publication on the platform. By comparing the file to online public databases, it seeks for existing similarities between files (equivalent to what Shazaam is for music, google image for images, plagiarism algorithms for texts and documents and automatic verifications on YouTube for videos). 

In other words, if the content is already available elsewhere over the web, it won’t be possible to upload it on 2PS, except by fulfilling special conditions leading to obtain a **fully verified official account** (See 1.7).
...
In this context, 2PS can be considered as a “patent registration application” for digital files. As for patents, users first deposit and go through the application process before making public use of their invention, digital files providers might want to protect their work by uploading it and going through the 2PS' publication process before sharing content anywhere else on internet. 
...
The file encryption protocol, once connected to the contract, will read the authorized password. If the user can provide the same password than the one available in the blockchain by hashing the Root ID of the file with his private key, then the ownership is confirmed and the file can be open.
...
Reclamations and complaints regarding ownership rights are made through the marketplace. Any users finding and being able to provide evidences than another user is committing fraud, property theft or any piracy-related issue can submit a complaint to 2PS. 
...
***Note**: During a **first phase of deployment, full ownership transfer will be disabled** and only streaming (read-only) will be possible. This is an attempt to protect real owners’ rights and customers. Indeed, an illegitimate user who would have passed through the identity check with a fake ID and through the similarity check with a file not being his/her actual property won’t be able to sell the file straight away.

In the case 2PS has to froze/delete this account for copyrights infringement, no user would have bought the file’s ownership from this account. Consequently, and in case of reclamation from the legitimate user, no sold copies representing a full ownership would be in circulation which makes it easier for 2PS to settle potential conflicts of interest.
...
The native owner choose to sell his short-story video with a contract passed for 100 copies given at full ownership and also decides to sell it only on 2PS at, say, 2000 2PS. The public found that this short-story is a highly valuable content and get positive reviews (5 stars) from all users whom watched it. The demand is rising and among the 100 that had the chance to acquirer one copy, only a few are willing to sell it back. In between, the native owner lost the original file because of a technical issue and is unable to emit new copies to fullfil the rising demand for his file. 

Thus, it is very likely that the high demand for this file will drive its price up quickly.
